      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="接口权限">
          <el-tree
            ...
            node-key="path_id"
            empty-text="加载中，请稍后"
            :props="pathDefaultProps"
            :check-strictly="pathCheckStrictly"
          >
            <span slot-scope="{ node, data }" class="custom-tree-node">
              <span>{{ node.label }}</span>
              <span v-if="data.type == 'J'"> -- -- </span>
              <span>{{ data.path }}</span>
            </span>
          </el-tree>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    /** 查询接口树结构 */
    getPathTreeselect(defaultKeys = null) {
      this.pathCheckStrictly = true
      listPath().then(response => {
        this.pathOptions = response.data
        if (defaultKeys && defaultKeys.length > 0) {
          this.$refs.path.setCheckedKeys(defaultKeys)
        }
        this.pathCheckStrictly = false
      })
    },
    /** 组合次菜单 button */
    ...
    // 所有接口节点数据
    getPathAllCheckedKeys() {
      // 目前被选中的菜单节点
      const checkedKeys = this.$refs.path.getHalfCheckedKeys()
      // 半选中的菜单节点
      const halfCheckedKeys = this.$refs.path.getCheckedKeys()
      checkedKeys.unshift.apply(checkedKeys, halfCheckedKeys)
      return checkedKeys
    },
    // 取消按钮
    ...
    reset() {
      ...
      if (this.$refs.path !== undefined) {
        this.$refs.path.setCheckedKeys([])
      }
      this.form = {
        role_id: undefined,
        role_name: undefined,
    ...
    handleUpdate(row) {
      ...
      this.pathDefaultKeys = row.path_ids || []
      this.$nextTick(() => {
        const buttons = row.buttons || []
        this.getMenuTreeselect(buttons, this.defaultKeys)
        this.getPathTreeselect(this.pathDefaultKeys)
      })
      this.form = {
        role_id: row.role_id,
    ...
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          this.form.path_ids = this.getPathAllCheckedKeys()
